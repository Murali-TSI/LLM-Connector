...
from ...base import BatchProcess, AsyncBatchProcess, BatchRequest, BatchResult, BatchStatus, BatchTimestamp
...
class OpenAIBatchProcess(BatchProcess):
    ...
    def create(self, *, file: Union[str, bytes, BinaryIO], completion_window: Literal["24h"] = "24h", endpoint: str = "/v1/chat/completions", metadata: Optional[dict] = None, **kwargs: Any) -> BatchRequest:
        """
        ...
        try:
            if isinstance(file, str):
                ...
                file_response = self._client.files.create(file=("batch.jsonl", file), purpose="batch")
            else:
    ...
    def result(self, job_id: str, **kwargs: Any) -> BatchResult:
        ...
        try:
            ...
            if batch.status != "completed":
                raise BatchError(f"Batch job is not completed. Current status: {batch.status}")
    ...
    def _to_batch_request(self, response) -> BatchRequest:
        ...
        return BatchRequest(
            ...
            request_counts=response.request_counts.model_dump() if response.request_counts else None,
        )
...
class OpenAIAsyncBatchProcess(AsyncBatchProcess):
    ...
    async def create(self, *, file: Union[str, bytes, BinaryIO], completion_window: Literal["24h"] = "24h", endpoint: str = "/v1/chat/completions", metadata: Optional[dict] = None, **kwargs: Any) -> BatchRequest:
        """
        ...
        try:
            if isinstance(file, str):
                ...
                file_response = await self._client.files.create(file=("batch.jsonl", file), purpose="batch")
            else:
                file_response = await self._client.files.create(file=file, purpose="batch")
